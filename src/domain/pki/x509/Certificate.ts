  ...
  static attach(runtime: X509Runtime, validators: X509Validators, h: X509Helpers): void {
    ...
    pki.certificateFromPem = function(pem: any, computeHash: any, strict: any) {
      ...
      if(msg.type !== 'CERTIFICATE' &&
        ...
        const error = new Error(
          ...
          'is not "CERTIFICATE", "X509 CERTIFICATE", or "TRUSTED CERTIFICATE".') as DerError;
    ...
    pki.publicKeyFromPem = function(pem: any) {
      ...
      if(msg.type !== 'PUBLIC KEY' && msg.type !== 'RSA PUBLIC KEY') {
        const error = new Error('Could not convert public key from PEM; PEM header ' +
          'type is not "PUBLIC KEY" or "RSA PUBLIC KEY".') as DerError;
        error.headerType = msg.type;
        throw error;
      }
      if(msg.procType && msg.procType.type === 'ENCRYPTED') {
    ...
    pki.certificationRequestFromPem = function(pem: any, computeHash: any, strict: any) {
      ...
      if(msg.type !== 'CERTIFICATE REQUEST') {
        const error = new Error('Could not convert certification request from PEM; ' +
          'PEM header type is not "CERTIFICATE REQUEST".') as DerError;
        error.headerType = msg.type;
        throw error;
      }
      if(msg.procType && msg.procType.type === 'ENCRYPTED') {
      ...
      cert.sign = function(key: any, md: any) {
        ...
        if(!algorithmOid) {
          const error = new Error('Could not compute certificate digest. ' +
            'Unknown message digest algorithm OID.') as DerError;
    ...
    pki.certificateFromAsn1 = function(obj: any, computeHash: any) {
      ...
      if(!asn1.validate(obj, validators.x509CertificateValidator, capture, errors)) {
        const error = new Error('Cannot read X.509 certificate. ' +
          'ASN.1 object is not an X509v3 Certificate.') as DerError;
        error.errors = errors;
        throw error;
      }
      ...
      cert.siginfo.parameters = readSignatureParameters(cert.siginfo.algorithmOid,
    ...
    pki.getTBSCertificate = function(cert: any) {
      ...
      const tbs = asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
        ...
        // serialNumber
        asn1.create(asn1.Class.UNIVERSAL, asn1.Type.INTEGER, false,
          c.util.hexToBytes(cert.serialNumber)),
        // signature
