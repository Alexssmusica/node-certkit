...
export type FillMissingExtensionFields = (
  e: X509Extension,
  options?: {cert?: X509Certificate}
) => X509Extension;

export function createFillMissingExtensionFields(
  deps: ExtensionFillDeps
  ...
  return function fillMissingExtensionFields(e: any, options?: any) {
    ...
    if(e.name === 'keyUsage') {
        ...
        if(altName.type === 7 && altName.ip) {
          ...
          if(value === null) {
            const error = new Error(
              'Extension "ip" value is not a valid IPv4 or IPv6 address.') as DerError;
            error.extension = e;
            throw error;
          }
      ...
      if(e.authorityCertIssuer) {
        const authorityCertIssuer = [
          asn1.create(asn1.Class.CONTEXT_SPECIFIC, 4, true, [
            dnToAsn1(e.authorityCertIssuer === true ?
      ...
      if(e.serialNumber) {
        const serialNumber = util.hexToBytes(e.serialNumber === true ?
