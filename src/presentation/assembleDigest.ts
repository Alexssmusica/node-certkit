...
import { Hmac } from '../domain/digest/Hmac.js';
import type { DigestAlgorithmRegistry } from '../domain/digest/DigestTypes.js';
...
export function assembleMd5(certkit: MutableCertkit): void {
  ...
  const md = certkit.md!;
  const algorithms = md.algorithms!;
  md.md5 = algorithms.md5 = md5;
}
...
export function assembleSha256(certkit: MutableCertkit): void {
  ...
  const md = certkit.md!;
  const algorithms = md.algorithms!;
  md.sha256 = algorithms.sha256 = sha256;
}
...
export function assembleSha512(certkit: MutableCertkit): void {
  ...
  const md = certkit.md!;
  const algorithms = md.algorithms!;
  md.sha512 = algorithms.sha512 = sha512;
