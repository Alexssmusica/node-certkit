...
var _pem = {
  ...
  describe('rsa', function() {
    ...
    function _pairCmp(pair1, pair2) {
      ...
      expect(pem1.privateKey).toBe(pem2.privateKey);
      expect(pem1.publicKey).toBe(pem2.publicKey);
    }
    ...
    it('should generate 512 bit key pair (async+workers)', async () => {
      await new Promise<void>((resolve) => {
        _genAsync({
          workers: -1
        }, function() {
    ...
    it('should generate same 512 bit key pair ' +
      '(prng+sync+purejs,prng+sync+purejs)', (ctx) => {
    ...
    (function() {
      ...
      function createTests(params) {
        ...
        it('should verify an rsa signature using a ' + keySize + '-bit key and PKCS#1 v1.5 padding', function() {
          ...
          expect(key.verify(md.digest().getBytes(), signature)).toBe(true);
        });
    ...
    describe('signature verification', function() {
      ...
      function _checkGoodDigestInfo(publicKey, S, skipTailingGarbage) {
        ...
        expect(publicKey.verify(md.digest().getBytes(), S, undefined, {
          _parseAllDigestBytes: !skipTailingGarbage,
          _skipPaddingChecks: true
        })).toBeTruthy();
      }
